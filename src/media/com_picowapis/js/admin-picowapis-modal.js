...
(function () {
  ...
  /**
    ...
    * View element calls jSelectPicowapi when a picowapi is clicked
    * jSelectPicowapi creates the link tag, sends it to the editor,
    * and closes the select frame.
    */

  window.jSelectPicowapi = function (id, title, catid, object, link, lang) {
    var hreflang = '';

    if (!Joomla.getOptions('xtd-picowapis')) {
      // Something went wrong
      window.parent.Joomla.Modal.getCurrent().close();
      return false;
    }

    var _Joomla$getOptions = Joomla.getOptions('xtd-picowapis'),
  ...
  document.addEventListener('DOMContentLoaded', function () {
    ...
    for (var i = 0, l = elements.length; l > i; i += 1) {
      ...
      elements[i].addEventListener('click', function (event) {
        ...
        if (functionName === 'jSelectPicowapi') {
          // Used in xtd_picowapis
